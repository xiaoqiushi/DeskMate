...
This is the DeskMate project — a Tauri desktop pet application with an agent monitoring platform.

IMPORTANT: DeskMate/ is the project root and has its own git repository. Do NOT operate on any parent directory or sibling directories.

- Frontend: TypeScript + React (Vite)
- Desktop shell: Rust (Tauri)
- Agent platform backend: Go

Before making any changes, confirm you are working in the correct project (DeskMate), not deskmate, gif-maker, or any other sibling directory.
...
2. **Hook script**: `~/.Codex/hooks/deskmate-codex-hook.sh` (Unix) / `.ps1` (Windows), regenerated by `install_claude_hooks()` via `install_codex_hooks()`.
3. **Supported events installed by DeskMate**: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PermissionRequest`, `Stop`, `StopFailure`, `SubagentStop`.
...
5. **Terminal focus extension** — `extensions/cursor/` contains a VS Code/Cursor extension (`deskmate.terminal-focus`) that exposes per-window HTTP metadata and focus endpoints. Auto-synced to `~/.cursor/extensions/` by `install_cursor_hooks()` on app startup.
...
- Identifier: `deskmate.terminal-focus`
...
- Install path: `~/.cursor/extensions/deskmate.terminal-focus-1.0.0/`
...
- Why HTTP over URI scheme: `cursor://` URI triggers a permission confirmation dialog; HTTP is silent and lets DeskMate distinguish multiple Cursor windows by port.
...
4. **Sub-agent completion sound suppression uses a `pending_agents` counter on `ClaudeSession`.** When CC spawns sub-agents (Agent tool), each completion triggers a `Stop` event. Without suppression, background agents cause 2-3 sounds per prompt. The counter tracks: `PreToolUse(Agent)` +1, `SubagentStop` -1. Sound only plays on `Stop` when `pending_agents == 0`. Key details:
   ...
   - `install_claude_hooks()` in `lib.rs` regenerates the hook script on every app startup, overwriting manual edits to `~/.claude/hooks/deskmate-hook.sh`. To persist hook changes, modify the template in `lib.rs`.
...
1. **The app no longer checks GitHub Releases directly.** `check_for_update()` in `frontend/src-tauri/src/lib.rs` reads a website-managed manifest instead. In dev it uses `http://[::1]:4321/update/latest.json`; in production it uses `https://raw.githubusercontent.com/xiaoqiushi/DeskMate/main/website/public/update/latest.json`.
...
5. **The UI can only show progress during the download phase.** The app downloads the DMG first, emits `update-progress` events, then spawns a detached helper script, exits, and lets the helper replace `/Applications/DeskMate.app` and relaunch. After the app exits there is no in-app UI, so that behavior is expected.
...
7. **Helper logs live in the system temp directory.** The updater writes files under `DeskMate-update-*` in the temp dir, including `install.log`. When debugging install failures, look there first.
...
2. **Build platform installers**:
   - **Windows**: `npx tauri build` in `frontend/`. Output: `frontend/src-tauri/target/release/bundle/nsis/DeskMate_<ver>_x64-setup.exe` (~35 MB).
   - **macOS**: `npx tauri build` on a Mac. Output: `frontend/src-tauri/target/release/bundle/dmg/DeskMate_<ver>_aarch64.dmg`. Requires signing & notarization for public distribution.
...
5. **Commit & push** the website changes so `raw.githubusercontent.com/xiaoqiushi/DeskMate/main/website/public/update/latest.json` serves the new version.
...
- SSH marker files are stored under `~/.ssh/DeskMate-ctl/` to fast-path connection checks.
...
- The correct approach: write a `.ps1` file and register the command as `powershell.exe -NoProfile -ExecutionPolicy Bypass -File 'C:/Users/xxx/.../deskmate-*-hook.ps1'` (forward slashes).
- `install_claude_hooks()` in `lib.rs` handles this automatically on startup. It also cleans up old `.cmd`-format hook entries.
- The cleanup logic (`has_our_hook`) matches any command containing `"deskmate-hook"` to remove both old and new formats.
...
- `npx tauri build` produces an NSIS installer `.exe` in `src-tauri/target/release/bundle/nsis/` and a standalone binary in `src-tauri/target/release/deskmate.exe`.
- Windows builds do not require code signing (users may see SmartScreen warnings).
- When `npx tauri dev` fails with "拒绝访问 (os error 5)", it means the old `deskmate.exe` process is still running and must be killed before recompilation can replace the binary.
