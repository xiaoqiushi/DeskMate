...
<p align="center">
  面向 AI 编程 agent 的桌面伙伴，支持任务状态监控、完成提示和编辑器窗口跳转。
</p>

> DeskMate 是基于 [OC-Claw](https://github.com/rainnoon/oc-claw) 的二次开发版本。

## 二开改动

- 默认 README 改为中文，英文文档保留在 [README.en.md](./README.en.md)。
- 应用名称、Bundle ID、安装脚本、官网、更新 manifest 和 Cursor 辅助扩展信息已改为 DeskMate。
- 任务完成提示改为右上角展示，不再自动弹出正上方面板。
- 正上方面板保留，改为手动触发，用于查看详情和权限相关信息。
- 完成提示和会话列表点击后优先跳转到对应编辑器窗口。
- Codex 会话支持通过 `codex://threads/<sessionId>` 跳转到对应线程。
- Cursor / VS Code 多窗口场景下按工作区路径匹配窗口。
- 打开项目文件夹功能保留为独立按钮。
- OpenClaw gateway 检测改为 lock/PID 检测，减少误报。
...
- 支持 Codex 线程 deep link、Cursor/VS Code 工作区窗口匹配，以及完成提示点击后回到对应编辑器
- 任务完成信息显示在右上角提示，提示里保留任务、项目、回复摘要和跳转入口
